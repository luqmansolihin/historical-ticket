    ...
    <div class="glass-card rounded-2xl shadow-2xl relative z-10 no-print flex-1 flex flex-col min-h-0 h-full overflow-hidden">
        ...
        <div class="px-3 py-1.5 bg-slate-900/90 border-b border-slate-800/80 flex items-center justify-between gap-2 shrink-0">
            <div class="text-xs text-slate-400 font-mono hidden sm:block">
                <i class="fa-solid fa-mouse-pointer text-purple-400 mr-1"></i> Double klik baris tabel untuk edit data akun
            </div>
            ...
            <div class="flex items-center gap-2 ml-auto">
                ...
                <a href="{{ route('users.create') }}" class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold text-white bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-400 hover:to-indigo-500 shadow-md shadow-purple-500/20 transition-all active:scale-95">
            ...
            <div x-ref="scrollContainer" class="overflow-auto flex-1 min-h-0" @scroll.passive="onScroll($event)">
                <table class="w-full text-left text-[9.5px] leading-tight text-slate-300 whitespace-nowrap border-collapse">
                    <thead class="bg-slate-900/95 text-[9px] uppercase font-bold text-slate-400 tracking-tight border-b border-slate-800 whitespace-nowrap sticky top-0 z-20 backdrop-blur-md">
                        <tr>
                            <!-- 1. ID & Nama Pengguna -->
                            <th class="py-1 px-2 whitespace-nowrap relative border-r border-slate-800/60 transition-colors" :class="activeFilters.name ? 'bg-purple-950/80 border-b-2 border-b-purple-400 text-purple-200' : ''" @click.outside="if (openPop === 'name') openPop = null">
                                <div class="flex items-center gap-1.5 justify-between">
                                    <button type="button" @click="toggleSort('name')" class="flex items-center gap-1 font-bold transition-colors cursor-pointer select-none group/sort" :class="getSortIndex('name') !== -1 ? 'text-purple-400 font-extrabold' : 'text-slate-300 hover:text-white'" title="Urutkan Nama Pengguna">
                                        ...
                                        <template x-if="getSortIndex('name') !== -1">
                                            <span class="inline-flex items-center gap-0.5 text-purple-400 font-bold text-[10px]">
                                                <i class="fa-solid" :class="getSortDir('name') === 'asc' ? 'fa-arrow-up-wide-short' : 'fa-arrow-down-wide-short'"></i>
                                                <span x-show="sorts.length > 1" class="text-[8px] bg-purple-500/20 px-1 py-0.2 rounded-full border border-purple-500/40 font-mono" x-text="getSortIndex('name') + 1"></span>
                                            </span>
                                        </template>
                                    </button>
                                    <button type="button" @click="openPop = (openPop === 'name' ? null : 'name')" class="p-1 rounded transition-colors" :class="activeFilters.name ? 'text-purple-300 bg-purple-500/30 ring-1 ring-purple-400/50 font-bold shadow-sm shadow-purple-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'" title="Filter Nama Pengguna">
                                        <i class="fa-solid" :class="activeFilters.name ? 'fa-filter text-purple-400 text-[11px]' : 'fa-caret-down text-xs'"></i>
                                    </button>
                                </div>
                                <div x-show="openPop === 'name'" x-cloak x-transition class="absolute z-50 left-0 mt-2 p-3 bg-slate-900 border border-slate-700/90 rounded-xl shadow-2xl space-y-3 text-left font-normal normal-case min-w-[220px]">
                                    ...
                                    <input type="text" name="search_name" value="{{ $searchName }}" placeholder="Cari nama pengguna..." class="w-full h-8 rounded-lg px-2.5 text-xs bg-slate-950 border border-slate-700/80 text-slate-200 placeholder-slate-500 focus:border-purple-400 focus:outline-none">
                                    <div class="flex items-center justify-end gap-2 pt-1 border-t border-slate-800/80">
                                        <button type="submit" class="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold shadow transition-colors">Terapkan</button>
                                    </div>
                                </div>
                            </th>

                            <!-- 2. Alamat Email -->
                            <th class="py-1 px-2 whitespace-nowrap relative border-r border-slate-800/60 transition-colors" :class="activeFilters.email ? 'bg-purple-950/80 border-b-2 border-b-purple-400 text-purple-200' : ''" @click.outside="if (openPop === 'email') openPop = null">
                                <div class="flex items-center gap-1.5 justify-between">
                                    <button type="button" @click="toggleSort('email')" class="flex items-center gap-1 font-bold transition-colors cursor-pointer select-none group/sort" :class="getSortIndex('email') !== -1 ? 'text-purple-400 font-extrabold' : 'text-slate-300 hover:text-white'" title="Urutkan Alamat Email">
                                        ...
                                        <template x-if="getSortIndex('email') !== -1">
                                            <span class="inline-flex items-center gap-0.5 text-purple-400 font-bold text-[10px]">
                                                ...
                                                <span x-show="sorts.length > 1" class="text-[8px] bg-purple-500/20 px-1 py-0.2 rounded-full border border-purple-500/40 font-mono" x-text="getSortIndex('email') + 1"></span>
                                            </span>
                                        </template>
                                    </button>
                                    <button type="button" @click="openPop = (openPop === 'email' ? null : 'email')" class="p-1 rounded transition-colors" :class="activeFilters.email ? 'text-purple-300 bg-purple-500/30 ring-1 ring-purple-400/50 font-bold shadow-sm shadow-purple-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'" title="Filter Email">
                                        <i class="fa-solid" :class="activeFilters.email ? 'fa-filter text-purple-400 text-[11px]' : 'fa-caret-down text-xs'"></i>
                                    </button>
                                </div>
                                <div x-show="openPop === 'email'" x-cloak x-transition class="absolute z-50 left-0 mt-2 p-3 bg-slate-900 border border-slate-700/90 rounded-xl shadow-2xl space-y-3 text-left font-normal normal-case min-w-[220px]">
                                    ...
                                    <input type="text" name="search_email" value="{{ $searchEmail }}" placeholder="Cari email..." class="w-full h-8 rounded-lg px-2.5 text-xs bg-slate-950 border border-slate-700/80 text-slate-200 placeholder-slate-500 focus:border-purple-400 focus:outline-none">
                                    <div class="flex items-center justify-end gap-2 pt-1 border-t border-slate-800/80">
                                        <button type="submit" class="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold shadow transition-colors">Terapkan</button>
                                    </div>
                                </div>
                            ...
                            <!-- 3. Role Akses -->
                            <th class="py-1 px-2 whitespace-nowrap relative border-r border-slate-800/60 transition-colors" :class="activeFilters.role ? 'bg-purple-950/80 border-b-2 border-b-purple-400 text-purple-200' : ''" @click.outside="if (openPop === 'role') openPop = null">
                                <div class="flex items-center gap-1.5 justify-between">
                                    <button type="button" @click="toggleSort('role')" class="flex items-center gap-1 font-bold transition-colors cursor-pointer select-none group/sort" :class="getSortIndex('role') !== -1 ? 'text-purple-400 font-extrabold' : 'text-slate-300 hover:text-white'" title="Urutkan Role Akses">
                                        ...
                                        <template x-if="getSortIndex('role') !== -1">
                                            <span class="inline-flex items-center gap-0.5 text-purple-400 font-bold text-[10px]">
                                                ...
                                                <span x-show="sorts.length > 1" class="text-[8px] bg-purple-500/20 px-1 py-0.2 rounded-full border border-purple-500/40 font-mono" x-text="getSortIndex('role') + 1"></span>
                                            </span>
                                        </template>
                                    </button>
                                    <button type="button" @click="openPop = (openPop === 'role' ? null : 'role')" class="p-1 rounded transition-colors" :class="activeFilters.role ? 'text-purple-300 bg-purple-500/30 ring-1 ring-purple-400/50 font-bold shadow-sm shadow-purple-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'" title="Filter Role">
                                        <i class="fa-solid" :class="activeFilters.role ? 'fa-filter text-purple-400 text-[11px]' : 'fa-caret-down text-xs'"></i>
                                    </button>
                                </div>
                                <div x-show="openPop === 'role'" x-cloak x-transition class="absolute z-50 left-0 mt-2 p-3 bg-slate-900 border border-slate-700/90 rounded-xl shadow-2xl space-y-3 text-left font-normal normal-case min-w-[200px]">
                                    ...
                                    <div class="space-y-1">
                                        <label class="flex items-center gap-2 px-2 py-1 rounded hover:bg-slate-800 cursor-pointer text-xs text-slate-200">
                                            <input type="radio" name="role" value="" {{ empty($roleFilter) ? 'checked' : '' }} class="text-purple-500">
                                            <span>Semua Role</span>
                                        </label>
                                        @foreach($roleOptions as $roleOpt)
                                            <label class="flex items-center gap-2 px-2 py-1 rounded hover:bg-slate-800 cursor-pointer text-xs text-slate-200">
                                                <input type="radio" name="role" value="{{ $roleOpt }}" {{ $roleFilter === $roleOpt ? 'checked' : '' }} class="text-purple-500">
                            ...
                            <!-- 4. Tanggal Terdaftar -->
                            <th class="py-1 px-2 whitespace-nowrap relative transition-colors" :class="activeFilters.date ? 'bg-purple-950/80 border-b-2 border-b-purple-400 text-purple-200' : ''" @click.outside="if (openPop === 'date') openPop = null">
                                <div class="flex items-center gap-1.5 justify-between">
                                    <button type="button" @click="toggleSort('created_at')" class="flex items-center gap-1 font-bold transition-colors cursor-pointer select-none group/sort" :class="getSortIndex('created_at') !== -1 ? 'text-purple-400 font-extrabold' : 'text-slate-300 hover:text-white'" title="Urutkan Tanggal Terdaftar">
                                        ...
                                        <template x-if="getSortIndex('created_at') !== -1">
                                            <span class="inline-flex items-center gap-0.5 text-purple-400 font-bold text-[10px]">
                                                ...
                                                <span x-show="sorts.length > 1" class="text-[8px] bg-purple-500/20 px-1 py-0.2 rounded-full border border-purple-500/40 font-mono" x-text="getSortIndex('created_at') + 1"></span>
                                            </span>
                                        </template>
                                    </button>
                                    <button type="button" @click="openPop = (openPop === 'date' ? null : 'date')" class="p-1 rounded transition-colors" :class="activeFilters.date ? 'text-purple-300 bg-purple-500/30 ring-1 ring-purple-400/50 font-bold shadow-sm shadow-purple-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'" title="Filter Tanggal Terdaftar">
                                        <i class="fa-solid" :class="activeFilters.date ? 'fa-filter text-purple-400 text-[11px]' : 'fa-caret-down text-xs'"></i>
                                    </button>
                                </div>
                                <div x-show="openPop === 'date'" x-cloak x-transition class="absolute z-50 left-0 mt-2 p-3.5 bg-slate-900 border border-slate-700/90 rounded-xl shadow-2xl space-y-3 text-left font-normal normal-case min-w-[260px]"
                                    ...
                                    <div class="text-xs font-semibold text-slate-300 border-b border-slate-800 pb-1.5 flex items-center justify-between">
                                        ...
                                        <i class="fa-regular fa-calendar-days text-purple-400"></i>
                                    </div>
                                    <div class="space-y-2 text-xs">
                                        <div>
                                            <label class="block text-[11px] font-medium text-slate-400 mb-1">
                                                <span class="text-purple-400 font-semibold">After</span> (Dari / Setelah):
                                            </label>
                                             <input type="date" name="date_after" x-model="dateAfter" @change="onAfterBeforeChange()" onclick="this.showPicker?.()" class="w-full h-8 rounded-lg px-2.5 text-xs bg-slate-950 border border-slate-700/80 text-slate-200 focus:border-purple-400 focus:outline-none font-mono cursor-pointer">
                                        </div>
                                        <div>
                                            <label class="block text-[11px] font-medium text-slate-400 mb-1">
                                                <span class="text-purple-400 font-semibold">Before</span> (Sampai / Sebelum):
                                            </label>
                                             <input type="date" name="date_before" x-model="dateBefore" @change="onAfterBeforeChange()" onclick="this.showPicker?.()" class="w-full h-8 rounded-lg px-2.5 text-xs bg-slate-950 border border-slate-700/80 text-slate-200 focus:border-purple-400 focus:outline-none font-mono cursor-pointer">
                                        </div>
                                        <div>
                                            <label class="block text-[11px] font-medium text-slate-400 mb-1">
